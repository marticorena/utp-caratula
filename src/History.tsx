...
type Props = { open: boolean; revision: number; onClose: () => void; onOpen: (data: StoredCover) => void };

export default function History({ open, revision, onClose, onOpen }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(`/api/covers?${new URLSearchParams({ q: query, offset: String(page * 20) })}`, { signal: controller.signal, cache: 'no-store' });
  ...
  }, [open, query, page, revision, retry]);

  async function remove(entry: Entry) {
    setBusy(entry.id); setError(''); setFeedback('');
    try {
      const response = await fetch(`/api/covers/${entry.id}`, { method: 'DELETE' });
  ...
  async function choose(entry: Entry, download: false | 'pdf' | 'docx') {
    ...
    try {
      ...
      const response = await fetch(`/api/covers/${entry.id}${download ? `/${download}` : ''}`, { cache: 'no-store' });
  ...
  return <dialog className="history-dialog" ref={dialog} onCancel={e => { if (busy) e.preventDefault(); else onClose(); }} aria-labelledby="history-title">
    ...
    <div className="history-results" aria-live="polite" aria-busy={loading}>
      ...
      {loading ? <div className="history-empty"><LoaderCircle className="spin" size={22}/><p>Buscando tus carátulas…</p></div> : !error && <>
        ...
        {!items.length && <div className="history-empty"><FileText size={30}/><h3>{query ? 'No hay coincidencias' : 'Tu historial empieza aquí'}</h3><p>{query ? 'Prueba con otra palabra, un nombre o un código.' : 'Usa Guardar o Descargar PDF para conservar tu primera carátula.'}</p></div>}
        {items.map(entry => <article key={entry.id} className="history-item"><span className="history-file"><FileText size={23}/></span><div className="history-info"><h3>{entry.title}</h3>{entry.course && <p>{entry.course}</p>}<time dateTime={entry.created_at}>{new Date(entry.created_at).toLocaleString('es-PE', { dateStyle: 'medium', timeStyle: 'short' })}</time></div><div className="history-actions"><button className="secondary-button" disabled={!!busy} onClick={() => choose(entry, false)}>{busy === entry.id ? 'Abriendo…' : 'Abrir en editor'}</button><button className="secondary-button history-download" aria-label={`Descargar PDF de ${entry.title}`} title="Descargar PDF guardado" disabled={!!busy} onClick={() => choose(entry, 'pdf')}><ArrowDownToLine size={16}/> PDF</button><button className="secondary-button history-download" aria-label={`Descargar DOCX de ${entry.title}`} disabled={!!busy} onClick={() => choose(entry, 'docx')}><ArrowDownToLine size={16}/> DOCX</button>{deleting === entry.id ? <div className="delete-confirm" role="group" aria-label={`Confirmar eliminación de ${entry.title}`}><span>¿Eliminar?</span><button className="secondary-button" disabled={!!busy} onClick={() => setDeleting(null)}>Cancelar</button><button className="danger-button" disabled={!!busy} onClick={() => remove(entry)}>{busy === entry.id ? 'Eliminando…' : 'Eliminar'}</button></div> : <button className="icon-button delete" aria-label={`Eliminar carátula ${entry.title}`} title="Eliminar carátula" disabled={!!busy} onClick={() => setDeleting(entry.id)}><Trash2 size={18}/></button>}</div></article>)}
    ...
    <p className="history-footnote">Al editar y guardar se crea una nueva versión. Las anteriores se conservan.</p>
  </dialog>;
}
